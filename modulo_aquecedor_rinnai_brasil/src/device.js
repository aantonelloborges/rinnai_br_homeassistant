...
const updateDeviceState = (retries = 0) => {
    ...
    return rinnaiApi.getState()
        .then(({ targetTemperature, isHeating, isPoweredOn }) => {
            ...
            console.log('[RINNAI API] updateDeviceState().response.targetTemperature', targetTemperature);
        })
        .catch(error => {
            if (retries < 5)
                return setTimeout(() => {
...
const updateParameters = (retries = 0) => {
    ...
    return rinnaiApi.getDeviceParams()
        .then(({
            ...
            console.log('[RINNAI API] updateParameters().response.inletTemperature', inletTemperature);
...
const updateConsumption = (retries = 0) => {
    return rinnaiApi.getConsumption()
        .then(({ water, gas, workingTime }) => {
            ...
            console.log('[RINNAI API] updateConsumption().response.gas', gas);
